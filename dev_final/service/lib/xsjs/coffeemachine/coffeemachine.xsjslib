var coffeemachine = function(connection) {
  ...
  */
  this.doGet = function(obj) {
    const result = connection.executeQuery(`SELECT * FROM "${CM_TABLE}"`);
    result.forEach(x => $.trace.error(JSON.stringify(x)));
    $.response.status = $.net.http.OK;
    $.response.setBody(JSON.stringify(result));
  };

  this.doPost = function(oCM) {
    oCM.cmid = getNextval("dev::cmid");
    const statement = STMT_INSTANCE.createPreparedInsertStatement(CM_TABLE, oCM);
    connection.executeUpdate(statement.sql, statement.aValues);
    connection.commit();
    $.response.status = $.net.http.CREATED;
    $.response.setBody(JSON.stringify(oCM));
  };

  this.doPut = function(oCM) {
    let sql = "";

    sql = `UPDATE "${CM_TABLE}" SET "name"='${oCM.name}', "ncups"='${oCM.ncups}'  WHERE "cmid"=${oCM.cmid};`;
  ...
  this.doDelete = function(cmid) {
    const statement = STMT_INSTANCE.createPreparedDeleteStatement(CM_TABLE, { cmid: cmid });
    connection.executeUpdate(statement.sql, statement.aValues)
    connection.commit();
    $.response.status = $.net.http.OK;
...
var statements = function() {
  ...
  this.createPreparedUpdateStatement = function(sTableName, oValueObject) {
    ...
    oResult.sql = `update "${sTableName}" set "name"='${oValueObject.name}', "ncups"='${oValueObject.ncups}' WHERE "cmid"=${oValueObject.cmid};`;
